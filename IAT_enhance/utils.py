...
def validation(model, val_loader, loss_function):
    ...
    for i, imgs in enumerate(val_loader):
        with torch.no_grad():
            ...
            '''
            low = np.array(low_img.cpu())
            high = np.array(high_img.cpu())
            model_output = np.array(enhanced_img.cpu())
        
            low = np.transpose(np.squeeze(low), (1, 2, 0))
            high = np.transpose(np.squeeze(high), (1, 2, 0))
            model_output = np.transpose(np.squeeze(model_output), (1, 2, 0))
            
            fig, axes = plt.subplots(1, 3)
            
            axes[0].imshow((low * 255).astype('int32'))
            axes[1].imshow((model_output * 255).astype('int32'))
            axes[2].imshow((high * 255).astype('int32'))
            
            axes[0].set_title('Model input')
            axes[1].set_title('Model output')
            axes[2].set_title('Ground truth')
            
            plt.savefig("model_validation/Sample_%d.png" % i)
            ...
            loss_value = loss_function(enhanced_img, high_img).item()
    ...
    print('\tThe validation loss is:', loss_mean)
    print()
    
    return SSIM_mean, PSNR_mean, loss_mean
